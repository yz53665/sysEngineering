...
m.S = Param(initialize=2.05) # 迎风面积
m.g = Param(initialize=9.8)
m.m = Param(initialize=1560)
m.cosTheta = Param(initialize=cos)
m.sinTheta = Param(initialize=sin)
m.rho = Param(initialize=1.293) # 空气密度
...
m.P = Var(m.tau,bounds=(0,205800),initialize=0)
...
def _ode4(m,t):
    if t == 0:
        return Constraint.Skip 
    return m.a[t] == (m.P[t] / (m.m*m.v[t]) - \
                    m.cosTheta * m.mu * m.g - \
                    m.sinTheta * m.g - \
                    m.C * m.rho * m.S * m.v[t]**2 * 0.5 / m.m)
m.ode4 = Constraint(m.tau, rule=_ode4)

def _ode5(m,t):
    if t == 0:
        return Constraint.Skip
    return m.P[t] == m.m * m.v[t]
m.ode5 = Constraint(m.tau, rule=_ode5)
...
print("final time = %6.2f" %(value(m.intJ)))
